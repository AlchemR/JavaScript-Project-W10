...
planck.testbed('Sandbox', function (testbed) {
  ...
  generateShot()
  window.onload = function () { ball1.addEventListener('drag', () => { console.log("event happened") }) }
  ...
  function keylistener() {
    ...
    if (testbed.activeKeys.right) {
      ...
      generateShot()
    } else if (testbed.activeKeys.z) { testbed.status('←/→: Accelerate car, ↑/↓: Change spring frequency') } else if (cameraLimitX(testbed.x)) {} else { testbed.x = ball1pos.x }
  ...
  testbed.step = function () {
    ...
    textOut()
    finishTouch()
    if (window.breakout1) { console.log("potnential world end trigger") ; console.log("try to delete world here") }
  };
